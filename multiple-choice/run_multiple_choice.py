...
def main():
    ...
    def preprocess_train_function(examples):
        ...
        paragraphs_idx = [
            idx + random.sample(set(range(len(context_json))) - set(idx), 7 - len(idx))
    ...
    if training_args.do_predict:
        ...
        for i, pred in enumerate(tqdm(preds)):
            ex = {
                ...
                "relevant": test_dataset["paragraphs"][i][pred]
                if pred < len(test_dataset["paragraphs"][i])
                else test_dataset["paragraphs"][0],
            }
